...
**注意**：Skyline 模式下 `getTabBar()` 是异步的，必须使用回调方式：

```typescript
// pages/xxx/xxx.ts
Page({
  // 打开弹窗时隐藏 TabBar（Skyline 模式下异步）
  onOpenPopup() {
...
2. **Skyline 模式下 getTabBar 是异步的**：必须使用回调方式 `this.getTabBar((tabBar) => { ... })`
